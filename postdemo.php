                ...
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    if (!$row = mysqli_fetch_assoc($resultl)){
                        if ($PaymentStatus== 'Paid') {
                            $UserStat = "Authorized Access";
                            $sql = "INSERT INTO logs (CardNumber, Name, SerialNumber,RoomNumber, DateLog, TimeIn, UserStat) VALUES (? ,?, ?, CURDATE(), CURTIME(), ?)";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    //Logout
                /*    else {
                      $sql = "SELECT * FROM logs WHERE CardNumber=?";
                      $result = mysqli_stmt_init($conn);
                      if (!mysqli_stmt_prepare($result, $sql)) {
                          echo "SQL_Error_Select_card";
                          exit();
                        ...
                        if ($row = mysqli_fetch_assoc($resultl)){
                          if(!empty($row['TimeOut'])){
                            ...
                            if (!mysqli_stmt_prepare($result, $sql)) {
                             ...
                             if ($PaymentStatus== 'Paid' && $row['TimeIn'] <= $TimeArrive) {
                                $UserStat = "Arrived on time and Authorized Access";
                            ...
                            elseif ($PaymentStatus== 'Paid' && $row['TimeIn'] > $TimeArrive){
                                $UserStat = "Arrived late and Authorized Acceess";
                                $sql="UPDATE logs SET TimeOut=CURTIME(), UserStat=? WHERE CardNumber=? AND DateLog=CURDATE()";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error_insert_logout1";
                            ...
                            elseif ($PaymentStatus== 'Due' && $row['TimeIn'] <= $TimeArrive) {
                                $UserStat = "Arrived on time and Access Denied";
                            }
                            elseif ($PaymentStatus== 'Due' && $row['TimeIn'] > $TimeArrive) {
                                $UserStat = "Arrived late and Access Denied";
                            }
                          }
                        }
                      }
